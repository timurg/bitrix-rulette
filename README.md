...
Компонент не использует языковые файлы, тему, правила и призы по умолчанию при инициализации.
Все надписи, тему, правила и призы нужно передавать явно.
...
- `text` plaint текст, для совместимости
- `html` для вывода разметки без экранирования, но лучше испольщзовать с острожностью, чтобы не поломать разметку
